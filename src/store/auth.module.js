...
const actions = {
  ...
  async [AUTH_TOKEN_REQUEST]({ commit }, pair) {
    commit(AUTH_TOKEN_REQUEST, pair)
    try {
      const jwt = await AuthService.requestEmailToken(pair)
      commit(AUTH_SUCCESS, jwt)
      router.push(router.history.current.query.redirect || '/')
      // commit(AUTH_TOKEN_REQUEST_SUCCESS, pair)
    } catch (e) {
